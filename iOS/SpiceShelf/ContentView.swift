...
struct ContentView: View {
    ...
    @State private var editingItem: SpiceItem?
    ...
    @ViewBuilder
    private func row(for entry: SpiceItem) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.name)
                .font(Theme.headlineFont)
                .foregroundColor(Theme.textPrimary)
            Text("Purchased: \(entry.purchaseDate)  \u{2022}  Shelf Life: \(entry.shelfLife)")
                .font(Theme.captionFont)
                .foregroundColor(Theme.textMuted)
        }
        .padding(.vertical, 4)
...
struct ItemEditView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Purchased (2026-01-15)", text: $field1)
                        .focused($focusedField, equals: .field1)
                        .accessibilityIdentifier("field1Field")
                    TextField("Shelf Life (12 months)", text: $field2)
            ...
            .navigationTitle(mode == .add ? "Add Spice" : "Edit Spice")
            .toolbar {
        ...
        .onAppear {
            if case .edit(let entry) = mode {
                name = entry.name
                field1 = entry.purchaseDate
                field2 = entry.shelfLife
                notes = entry.notes
            }
        }
    }
    ...
    private func save() {
        switch mode {
        case .add:
            store.add(name: name, purchaseDate: field1, shelfLife: field2, notes: notes)
        case .edit(var entry):
            entry.name = name
            entry.purchaseDate = field1
            entry.shelfLife = field2
            entry.notes = notes
            store.update(entry)
        }
        dismiss()
